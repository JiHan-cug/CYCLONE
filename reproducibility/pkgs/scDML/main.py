...
class scDMLModel:
    ...
    def calculate_similarity(self,K_in=5,K_bw=10,K_in_metric="cosine",K_bw_metric="cosine"):
        ...
        if(self.nbatch<10):
            ...
            knn_intra_batch_approx=get_dict_mnn(data_matrix=self.emb_matrix,batch_index=self.batch_index,k=K_in,flag="in",metric=K_in_metric,approx=False,return_distance=False,verbose=self.verbose,log=self.log)
            knn_intra_batch=np.array([list(i)for i in knn_intra_batch_approx])
            if(self.verbose):
                self.log.info("appoximate calculate MNN Pair inter batch...")
            mnn_inter_batch_approx=get_dict_mnn(data_matrix=self.emb_matrix,batch_index=self.batch_index,k=K_bw,flag="out",metric=K_bw_metric,approx=False,return_distance=False,verbose=self.verbose,log=self.log)
            ...
            if(self.verbose):
                ...
                self.log.info("appoximate calculate KNN Pair intra batch...")
            knn_intra_batch_approx=get_dict_mnn_para(data_matrix=self.emb_matrix,batch_index=self.batch_index,k=K_in,flag="in",metric=K_in_metric,approx=False,return_distance=False,verbose=self.verbose,log=self.log)
            knn_intra_batch=np.array(knn_intra_batch_approx)
            if(self.verbose):
                self.log.info("appoximate calculate MNN Pair inter batch...")
            mnn_inter_batch_approx=get_dict_mnn_para(data_matrix=self.emb_matrix,batch_index=self.batch_index,k=K_bw,flag="out",metric=K_bw_metric,approx=False,return_distance=False,verbose=self.verbose,log=self.log)

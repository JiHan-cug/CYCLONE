...
Convert(paste0("../../data/Simulation/simulation.h5ad"), dest="h5seurat",
        assay = "RNA",
        overwrite=F)
seurat_object <- LoadH5Seurat(paste0("../data/simulation.h5seurat"))
...
write.csv(clust, paste0("simulation_seurat_clust.csv"))

results = data.frame(xpca,
                     simulate.combined@meta.data[["Group"]],
                     simulate.combined@meta.data[["Batch"]]))
write.csv(results, paste0("simulation_seurat_emb.csv"))

...
def my_func(adata):
    ...
    adata_corr = []
    genes_ = []
    all_batch = list(set(adata.obs['tech']))
    for b in all_batch:
        adata_corr.append(adata.X[adata.obs['tech'] == b, :])
...
if __name__ == '__main__':
    ...
    adata_inted.obs['batch'] = np.array(adata.obs['tech'])

    adata_inted.write_h5ad("pancreas_scanorama_emb.h5ad")

...
Convert(paste0("../../data/Human Lung Cell/Lung_atlas_public.h5ad"), dest="h5seurat",
        assay = "RNA",
        overwrite=F)
seurat_object <- LoadH5Seurat(paste0("../data/lung.h5seurat"))
...
write.csv(clust, paste0("Bct-s_seurat_clust.csv"))

results = data.frame(xpca,
                     simulate.combined@meta.data[["cell_type"]],
                     simulate.combined@meta.data[["batch"]]))
write.csv(results, paste0("lung_seurat_emb.csv"))

...
from train import cyclone_noupdate
...
adata, nmi, ari, k, run_time, BASW, BER, mnn_index = cyclone_noupdate(adata,
                                                                      batch_size=256,
                                                                      vae_z_dim=32,
                                                                      h_dim=16,
...
z = adata.obsm['cyclone_noupdate_emb']
y_pred = adata.obs['cyclone_noupdate_cluster']

corrd = pd.DataFrame(z)
adata_corrd = ad.AnnData(corrd, obs=adata.obs, dtype='float64')
adata_corrd.obsm['X_latent'] = adata.obsm['cyclone_noupdate_emb']
adata_corrd.obs['celltype'] = np.array(adata.obs['celltype'])
adata_corrd.obs['batch'] = np.array(adata.obs['Batch'])
adata_corrd.write_h5ad('Simulation_cyclone_noupdate_emb.h5ad')

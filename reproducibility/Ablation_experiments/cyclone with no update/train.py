...
def cyclone_noupdate(adata,
    ...
    """
        ...
        -------
        adata
            AnnData object of scanpy package. Embedding and clustering result will be stored in adata.obsm['cyclone_noupdate_emb']
            and adata.obs['cyclone_noupdate_cluster']
        nmi
        ...
        K
            ...
            we can adjust the resolution to the true number of cells within the function `cyclone_noupdate`
        run_time
    ...
    VAE_model = Con_l(adata.shape[1], adata.obs.domain_number[0], device=device,
                      z_dim=vae_z_dim, h_dim=h_dim, encode_layers=encode_layers, decode_layers=decode_layers).to(device)
    ...
    KLD_Loss= ELOBkldLoss().to(device)

    optimizer_vae = torch.optim.Adam(VAE_model.parameters(), lr=lr_vae)

    ############################# Train of cyclone_noupdate  #############################
    ...
    for epoch in range(train_epoch):
        ...
        #  No update
        if epoch == 0:
            adata.obsm['vae_emb'] = VAE_model.EncodeAll(adata.X, np.array(batch_onehot)).cpu().numpy()
        ...
        sum_con, sum_e, sum_mse, sum_kld = 0, 0, 0, 0
        ...
        for data in con_pairs_loader:
            ...
            z_mu, z_logvar, z, q, k, x_x = VAE_model(x, b, x1, x2)
            ...
            loss_e = con_w * loss_con + loss_mse_x + kld_w * loss_kld

            sum_e += loss_e.item()
            sum_con += loss_con.item()
            sum_mse += loss_mse_x.item()
            sum_kld += loss_kld.item()
        ...
        print(
            'train epoch [{}/{}]. MSE loss:{:.4f}, KLD loss:{:.4f}, Contrast loss:{:.4f}, total loss:{:.4f}'.format(
                epoch + 1, train_epoch, sum_mse / len(con_pairs_loader),
                sum_kld / len(con_pairs_loader),
    ...
    ################################## Calculate metrics ##################################
    cyclone_noupdate_emb = VAE_model.EncodeAll(adata.X, np.array(batch_onehot)).cpu().numpy()
    adata_l = sc.AnnData(cyclone_noupdate_emb)
    ...
    adata.obsm['feat'] = cyclone_noupdate_emb
    ...
    ############################   Return results   #########################
    adata.obsm['cyclone_noupdate_emb'] = cyclone_noupdate_emb
    adata.obs['cyclone_noupdate_cluster'] = y_pred

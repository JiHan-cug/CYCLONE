...
def cyclone(adata,
    ...
    ################################## Calculate metrics ##################################
    cyclone_emb = VAE_model.EncodeAll(adata.X, np.array(batch_onehot)).cpu().numpy()
    adata_l = sc.AnnData(cyclone_emb)
    ...
    ############################   Return results   #########################
    adata.obsm['cyclone_emb'] = cyclone_emb
    adata.obs['cyclone_cluster'] = y_pred
